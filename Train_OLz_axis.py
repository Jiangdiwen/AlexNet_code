...
LastEditTime: 2023-09-16 22:27:38
Description: 
...
from Dataloader.dataloader_OLz_axis import load_npydata
from Dataloader.dataloader_npy import CustomDataset
from Model.CameraRegressor import AlexNet_Regressor_Z, CombinedModel, AlexNetFeatures, CameraParamRegressor_1
...
def normalize_0_1_label(labels, 
                         para_ranges = np.array([[-30, 30], [-30, 30], [-90, 90],
                                                 ...
                                                 [0, 2]])):
    # 对labels的每一列参数进行归一化
    normalized_labels = np.zeros_like(labels, dtype=np.float32)  # 这里指定dtype为np.float32
    normalized_labels = (labels - para_ranges[0]) / (para_ranges[1] - para_ranges[0])
    return normalized_labels.astype(np.float32).reshape(-1, 1)
...
if __name__ == '__main__':
    ...
    print_message(f'use proj_data : {Proj_data_path}')
    ...
    regressor_model = CameraParamRegressor_1(input_size=256 * 6 * 6).float()
    #AlexNet_Regressor = AlexNet_Regressor_Z().float()
    # ----------------------Task_parameters---------------------------------
    #num_epochs = 1500 
    num_epochs = 800
    num_Task = '_OLz_07003_divide_model'
    ...
    # 定义优化器
    #features_optimizer = optim.SGD(feature_extraction.parameters(), lr=lr_features, momentum=0.9)
    ...
    for epoch in range(num_epochs):
        ...
        #torch.save(AlexNet_Regressor.state_dict(), current_model_path)
    ...
    # torch.save(AlexNet_Regressor.state_dict(), final_model_path)
    time1 = time.time()
