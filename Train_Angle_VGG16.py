...
from Model.VGG_model import Vgg16_net
...
def Set_Task_name(num_Task, num_epochs, num_train_data, batch_size, lr_features):
    # 获取当前日期时间
    current_datetime = datetime.now()   
    today_month_day = current_datetime.strftime("%m-%d")
    Task_name = f'{today_month_day}_Task{num_Task}lr_features{lr_features}_{batch_size}batch_{num_epochs}epochs_{num_train_data}trainDatas'
...
if __name__ == '__main__':
    ...
    data_transform = transforms.Compose([
        ...
        transforms.Resize((224, 224), antialias=True),   # 调整大小为AlexNet输入尺寸
        # 可以添加其他预处理转换
    ...
    VGG16_model = Vgg16_net(num_OutputPara=3)
    # ----------------------Task_parameters---------------------------------
    #num_epochs = 1500 
    num_epochs = 1500
    num_Task = '_VGG16_test3_weight_decay'
    num_train_data = len(train_data)        
    lr_regressor = 1e-05
    lr_features = 1e-06
    # lr_regressor = 1e-05
    # lr_features = 1e-03
    Task_name = Set_Task_name(num_Task, num_epochs, num_train_data, batch_size, lr_features)
    # 定义损失函数和优化器
    criterion = nn.MSELoss()    
    # 定义优化器
    optimizer = optim.Adam(VGG16_model.parameters(), lr=lr_features, weight_decay=0.001)
       
    #optimizer = optim.SGD(regressor_model.parameters(), lr=lr, momentum=0.9)
    # 设置学习率衰减因子
    lr_decay_factor = 0.1
    lr_scheduler_0 = optim.lr_scheduler.StepLR(optimizer, step_size=200, gamma=lr_decay_factor)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                vali_outputs = VGG16_model(images)
        ...
        torch.save(VGG16_model.state_dict(), current_features_model_path)
        ...
        if epoch_loss < best_train_loss:
            ...
            torch.save(VGG16_model.state_dict(), best_epoch_model_features_path)
        ...
        if val_epoch_loss < best_val_loss:
            ...
            torch.save(VGG16_model.state_dict(), best_vali_model_features_path)
