...
LastEditTime: 2023-09-14 10:24:00
Description: 
...
from Model.CameraRegressor import CameraParamRegressor_6, CameraParamRegressor_7, CameraParamRegressor_8, CombinedModel
...
if __name__ == '__main__':
    ...
    Proj_data_path = '/home/jyx/Jiangyanxin/Reg_DSA/AlexNet_Regression_Model/data/Train_datas/0829_skull_smallweiyi_4200datas.npy'
    data, labels = load_npydata(Proj_data_path)  # 二维图像矩阵的列表，每个元素是一个二维矩阵
    # 在切换模型为验证模式之前定义参数名列表，用于后续的打印
    param_names = ['Rx', 'Ry', 'Rz', 'Dx', 'Dy', 'Dz', 'side', 'd']
    ...
    #     print(f'max:{np.max(normal_labels[:,i])}')
    #     print(f'min:{np.min(normal_labels[:,i])}')
    ...
    regressor_model = CameraParamRegressor_8(input_size=feat_size[0] * feat_size[1] * feat_size[2]).float()
    # ----------------------Task_parameters---------------------------------
    #num_epochs = 1500 
    num_epochs = 3000
    num_Task = '_8paras_smallweiyi'
